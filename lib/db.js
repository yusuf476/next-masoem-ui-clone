...
const dataDirectory = process.env.VERCEL
  ? path.join(os.tmpdir(), "masoem-market-data")
  : bundledDataDirectory;
const bundledDataFile = path.join(bundledDataDirectory, "store.json");
...
const SUPABASE_REQUEST_TIMEOUT_MS = 4 * 1000;
const fallbackBlobPath = "app-store/store.json";
...
function getSupabaseUrl() {
  const publicUrl = stripWrappingQuotes(process.env.NEXT_PUBLIC_SUPABASE_URL);
  const serverUrl = stripWrappingQuotes(process.env.SUPABASE_URL);
  return (publicUrl || serverUrl || "").replace(/\/$/, "");
}

function getSupabaseServiceRoleKey() {
  return stripWrappingQuotes(process.env.SUPABASE_SERVICE_ROLE_KEY);
}

function getBlobReadWriteToken() {
  return stripWrappingQuotes(process.env.BLOB_READ_WRITE_TOKEN);
}
...
export function getDataSourceLabel() {
  if (forcedLocalStoreReason) {
    return isBlobConfigured() ? "Vercel Blob Store (Supabase fallback)" : "Local JSON Store (Supabase fallback)";
  }
  ...
  if (isBlobConfigured()) {
    return "Vercel Blob Store";
  }

  return "Local JSON Store";
}
...
async function readBundledStoreSeed() {
  try {
...
async function ensureLocalStore() {
  ...
  try {
    ...
    const initialStore = await readBundledStoreSeed();
...
async function readBlobStore() {
  const result = await getBlob(fallbackBlobPath, {
    access: "private",
...
async function writeBlobStore(store) {
  await putBlob(fallbackBlobPath, JSON.stringify(normalizeStore(store), null, 2), {
    access: "private",
...
async function readFallbackStore() {
  if (isBlobConfigured()) {
    try {
      ...
      console.warn("Blob read failed, falling back to local store:", getErrorMessage(error));
    }
  }
...
async function writeFallbackStore(store) {
  if (isBlobConfigured()) {
    try {
      ...
      console.warn("Blob write failed, falling back to local store:", getErrorMessage(error));
    }
  }
...
async function seedSupabaseCatalogIfEmpty(categories, products) {
  ...
  const localSeed = await readBundledStoreSeed();
...
export async function readStore() {
  if (isSupabaseConfigured() && (await ensureSupabaseReady())) {
    try {
      ...
      await persistFallbackReason(forcedLocalStoreReason);
      console.warn("Supabase read failed, falling back to runtime store:", getErrorMessage(error));
    }
  }
